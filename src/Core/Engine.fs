...
module Engine =
    ...
    let request<'a, 'b> ep id  (obj : 'a) =
        ...
        |> Promise.onFail (fun r ->
            Browser.console.error ("[TDDSTUD10-ENGINE-ERR]", id, ep, r)
            null |> unbox
        )
        |> Promise.map(fun r ->
            try
        ...
        Promise.create (fun resolve reject ->
            let child =
                if Process.isMono () then
                    let mono = workspace.getConfiguration().get("TddStud10.monoPath", "mono")
            ...
            |> Process.onOutput (fun n ->
                ...
                if isStartedMessage then
                    Browser.console.log ("[TDDSTUD10-ENGINE-SIG] started message?", isStartedMessage)
                    service <- Some child
                    resolve child
                else
